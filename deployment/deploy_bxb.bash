...
cd /usr/local/src
sudo git clone https://github.com/BixBite-project/bixbite.git
cd bixbite
sudo make -j$(nproc)
sudo cp ~/nodejs-pool/deployment/bixbite.service /lib/systemd/system/
sudo useradd -m bxbdaemon -d /home/bxbdaemon
BLOCKCHAIN_DOWNLOAD_DIR=$(sudo -u bxbdaemon mktemp -d)
...
mysql -u root --password=$ROOT_SQL_PASS < deployment/basebxb.sql
