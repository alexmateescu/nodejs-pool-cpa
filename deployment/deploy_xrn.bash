...
sudo DEBIAN_FRONTEND=noninteractive apt-get -y install git python-virtualenv python3-virtualenv curl ntp build-essential screen cmake pkg-config libboost-all-dev libevent-dev libunbound-dev libminiupnpc-dev libunwind8-dev liblzma-dev libgtest-dev libldns-dev libexpat1-dev mysql-server lmdb-utils libzmq3-dev
cd ~
git clone https://github.com/gokul57/nodejs-pool-1.git nodejs-pool # Change this depending on how the deployment goes.
cd /usr/src/gtest
sudo cmake .
sudo make
...
cd /usr/local/src
sudo git clone https://github.com/Saronite/saronite.git
cd saronite
sudo make -j$(nproc)
sudo cp ~/nodejs-pool/deployment/saronite.service /lib/systemd/system/
sudo useradd -m xrndaemon -d /home/xrndaemon
sudo systemctl daemon-reload
sudo systemctl enable saronite
sudo systemctl start saronite
...
cd ~
git clone https://github.com/gokul57/poolui-xtl.git poolui
cd poolui
npm install
...
mysql -u root --password=$ROOT_SQL_PASS < deployment/basexrn.sql
