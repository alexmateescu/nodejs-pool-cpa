...
let paymentQueue = async.queue(function (paymentDetails, callback) {
    ...
    function getbalance() {
	    ...
	    console.log("body:",body);
            ...
            console.log("Current wallet balance is " + global.support.coinToDecimal(body.result.availableBalance + parseInt(body.result.lockedAmount)) + " with " + global.support.coinToDecimal(body.result.availableBalance) + " unlocked balance");
	    //console.log("balance:", body.result.availableBalance + parseInt(body.result.lockedAmount));
            //console.log("unlocked:", body.result.availableBalance);
    ...
    this.makePaymentWithID = function () {
        ...
        paymentQueue.push(paymentDetails, function (body) {
            if (typeof body.transactionHash !== 'undefined') {
                console.log("[*] Successful payment to " + identifier + " of " + global.support.coinToDecimal(amount) + " XMLC (fee " + global.support.coinToDecimal(fee) + " - " + global.support.coinToDecimal(global.config.payout.fee) + " = " + global.support.coinToDecimal(fee - global.config.payout.fee) + ") with tx_hash " + body.transactionHash.match(hexChars)[0]);
    ...
    this.trackPayment = function () {
	    ...
	    console.log("rows:",rows);
            ...
            global.support.sendEmail(rows[0].email,
                sprintf("Your %(payment_amount)s XMLC payment was just performed", emailData),
                sprintf(
                    "Your payment of %(payment_amount)s XMLC (with tx fee %(fee)s XMLC) to %(address)s wallet was just performed and total due was decreased by %(amount)s XMLC.\n" +
...
function makePayments() {
    if (is_full_stop) {
        console.log("Dropping all new payment creation");
        return;
    }
    if (paymentQueue.idle() === false) {
        console.log("Payment queue is not empty so dropping all new payment creation");
        return;
    }
    if (shapeshiftQueue.idle() === false) {
        console.log("Shapeshift payment queue is not empty so dropping all new payment creation");
        return;
    }
    if (xmrToQueue.idle() === false) {
        console.log("xmr.to payment queue is not empty so dropping all new payment creation");
        return;
    }
	...
	console.log("paying rows:",rows);
	console.log("min payout:",global.support.decimalToCoin(global.config.payout.walletMin));
        ...
        rows.forEach(function (row) {
            ...
            global.mysql.query("SELECT payout_threshold FROM users WHERE username = ?", [payee.id]).then(function (userRow) {
                ++ roundCount;
                let threshold = global.support.decimalToCoin(global.config.payout.walletMin);
                        ...
                        paymentQueue.unshift(paymentDetails, function (body) {  //jshint ignore:line
                            ...
                            if (typeof body.transactionHash !== 'undefined') {
                                ...
                                paymentDetails.transfers.forEach(function (payeeItem) {
                                    ...
                                    console.log("[**] Successful payment to " + payeeItem.address + " for " + global.support.coinToDecimal(payeeObjects[payeeItem.address].amount) + " XMLC (fee " + global.support.coinToDecimal(payeeObjects[payeeItem.address].fee) + ")");
                                });
                                console.log("[*] Successful payment to multiple miners of " + global.support.coinToDecimal(totalAmount) + " XMLC (fee " + global.support.coinToDecimal(totalFee) + " - " + global.support.coinToDecimal(global.config.payout.fee) + " = " + global.support.coinToDecimal(totalFee) + ") with tx_hash " + body.transactionHash.match(hexChars)[0]);
                                global.mysql.query("INSERT INTO transactions (bitcoin, address, payment_id, xmr_amt, transaction_hash, mixin, fees, payees) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
