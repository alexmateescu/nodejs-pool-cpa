...
function updateWalletStats() {
    async.waterfall([
		...
		console.log(body)
                if (body.result) {
                    return callback(null, {
                        balance: body.result.availableBalance + parseInt(body.result.lockedAmount),
                        unlocked: body.result.availableBalance,
                        ts: Date.now()
                    });
                } else {
        ...
        function (state, callback) {
            // Todo: Implement within the coins/<coin>.js file.
            global.support.rpcNewDaemon('getheight', [],function (body) {
		        console.log(body)
                if (body) {
